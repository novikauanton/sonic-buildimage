...
ssize_t iccp_send_to_mclagsyncd(uint8_t msg_type, char *send_buff, uint16_t msg_len)
{
    ...
    ICCPD_LOG_ERR(__FUNCTION__, "Sending to mclagsyncd...");
    if (sys->sync_fd)
    {
        while (msg_len > 0)
        {
        ...
        ICCPD_LOG_ERR(__FUNCTION__, "sys->syncd_fd is NULL");
    }
    ...
    uint8_t                 *system_id)
{
    ...
    if (sys->sync_fd) {
        ICCPD_LOG_NOTICE(__FUNCTION__, "Calling iccp_send_to_mclagsyncd");
        rc = iccp_send_to_mclagsyncd(msg_hdr->type, msg_buf, msg_hdr->len);
    }
